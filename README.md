<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=300&color=0:0f0c29,50:302b63,100:24243e&text=Víctor%20Gómez%20Molina&fontSize=50&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Full-Stack%20Developer%20·%20Backend%20Engineer%20·%20API%20Architect&descAlignY=55"/>

</div>

---

<div align="center">

# ⚡ FULL STACK DEVELOPER

### Especializado en arquitecturas modernas, APIs seguras y sistemas escalables

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1000&color=00F7FF&center=true&vCenter=true&width=1000&lines=Node.js+%7C+Express+%7C+MongoDB;Angular+%7C+React+%7C+JWT+Authentication;REST+API+Architecture;Clean+Code+%7C+Scalable+Systems;Backend+Engineering+%7C+Frontend+Integration" />

<br>

<img src="https://komarev.com/ghpvc/?username=Mugicat05&label=VISITORS&color=0e75b6&style=for-the-badge"/>

<img src="https://img.shields.io/github/followers/Mugicat05?style=for-the-badge&logo=github&color=171515"/>

<img src="https://img.shields.io/github/stars/Mugicat05?style=for-the-badge&logo=github&color=yellow"/>

<img src="https://img.shields.io/badge/Focus-FullStack-blueviolet?style=for-the-badge"/>

</div>

---

# 🧠 SYSTEM.INFO

```yaml
name: Víctor Gómez Molina
role: Full Stack Developer
specialization:
  - Backend Architecture
  - REST APIs
  - JWT Authentication
  - MongoDB Persistence
  - Angular & React Frontends

current_focus:
  - Clean Architecture
  - Secure Systems
  - Scalable APIs
  - Modern Frontend Design

philosophy:
  - Maintainable code
  - Functional fidelity
  - Technical consistency
  - Reproducible systems
```

---

# ⚔️ TECH STACK

<div align="center">

## 🚀 Core Technologies

<img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,javascript,typescript,angular,react,bootstrap,html,css,sass,git,github,vscode,linux,postman&perline=8" />

</div>

---

# 🧩 BACKEND ENGINEERING

<div align="center">

| Architecture | Security | Database | APIs |
|---|---|---|---|
| Modular Systems | JWT Access Tokens | MongoDB | RESTful APIs |
| Middleware Design | Refresh Tokens | Mongoose ODM | CRUD Operations |
| Error Handling | Route Protection | Schema Validation | API Scalability |
| Clean Structure | Session Management | Data Persistence | Backend Logic |

</div>

---

# 🎨 FRONTEND DEVELOPMENT

<div align="center">

| Angular | React | UI/UX |
|---|---|---|
| Reactive Forms | Reusable Components | Responsive Interfaces |
| Services & Routing | State Management | Bootstrap Integration |
| Component Architecture | Hooks & Logic | Modern Layout Systems |

</div>

---

# 🏆 FEATURED PROJECTS

<div align="center">

<a href="https://github.com/Mugicat05/Tienda-HSN-React-and-Node">
<img width="420" src="https://github-readme-stats.vercel.app/api/pin/?username=Mugicat05&repo=Tienda-HSN-React-and-Node&theme=tokyonight&hide_border=true&border_radius=20"/>
</a>

<a href="https://github.com/victorgm0502-del/FoodExpressAPI">
<img width="420" src="https://github-readme-stats.vercel.app/api/pin/?username=victorgm0502-del&repo=FoodExpressAPI&theme=tokyonight&hide_border=true&border_radius=20"/>
</a>

</div>

---

# 🛒 TIENDA HSN · FULL STACK APP

<div align="center">

<img src="https://img.shields.io/badge/STATUS-IN%20DEVELOPMENT-success?style=for-the-badge"/>

</div>

Aplicación full-stack orientada a autenticación segura y gestión avanzada de usuarios y direcciones.

### ⚡ Características

- Arquitectura frontend/backend desacoplada
- Backend Node.js + Express
- Frontend React
- Persistencia MongoDB
- JWT Authentication
- Control avanzado de errores
- Middleware personalizado
- Gestión segura de sesiones

### 🛠️ Stack

<p align="center">
<img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB"/>
<img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white"/>
<img src="https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white"/>
<img src="https://img.shields.io/badge/JWT-black?style=for-the-badge&logo=jsonwebtokens"/>
</p>

---

# 🍔 FOODEXPRESS API

API REST modular para gestión de pedidos, autenticación y control de stock.

### 🔥 Incluye

- CRUD avanzado
- Arquitectura escalable
- Validaciones robustas
- Middleware personalizado
- Persistencia eficiente
- Gestión modular
- Seguridad JWT

---
# 📊 GITHUB ANALYTICS

<div align="center">

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=Mugicat05&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117&title_color=00F7FF&icon_color=00F7FF&text_color=FFFFFF&border_radius=20"/>

<img width="49%" src="https://github-readme-streak-stats.herokuapp.com/?user=Mugicat05&theme=radical&hide_border=true&background=0D1117&ring=00F7FF&fire=00F7FF&currStreakLabel=00F7FF&border_radius=20"/>

<br><br>

<img width="42%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Mugicat05&layout=compact&theme=radical&hide_border=true&bg_color=0D1117&title_color=00F7FF&text_color=FFFFFF&border_radius=20"/>

</div>

---

# ⚡ ACTIVITY GRAPH

<div align="center">

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=Mugicat05&bg_color=0d1117&color=00f7ff&line=00f7ff&point=ffffff&area=true&hide_border=true&radius=16"/>

</div>

---

# 🐍 CONTRIBUTION SNAKE

<div align="center">

<img width="100%" src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake-dark.svg"/>

</div>

---

# 🧠 DEVELOPMENT PHILOSOPHY

<div align="center">

> ### “Software should be scalable, maintainable and technically defendable.”

</div>

Cada proyecto se desarrolla bajo principios de:

- Clean Architecture
- Modular Design
- Technical Consistency
- Functional Reproducibility
- Secure Authentication
- Backend Scalability
- Maintainable Systems

---

# 🎮 EXTRA TECHNICAL INTERESTS

<div align="center">

| System Optimization | Emulation Engineering |
|---|---|
| RetroArch | Flycast |
| PPSSPP | Emulator Configuration |
| Performance Tuning | Compatibility Optimization |

</div>

---

# 🌐 CONNECT

<div align="center">

<a href="https://github.com/Mugicat05">
<img src="https://img.shields.io/badge/GitHub-Mugicat05-171515?style=for-the-badge&logo=github"/>
</a>

<br><br>

<a href="mailto:tuemail@example.com">
<img src="https://img.shields.io/badge/EMAIL-CONTACT-red?style=for-the-badge&logo=gmail"/>
</a>

</div>

---

<div align="center">

# ⚡ BUILDING MODERN SOFTWARE

<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight"/>

<br><br>

<img src="https://capsule-render.vercel.app/api?type=waving&height=180&section=footer&color=0:0f0c29,50:302b63,100:24243e"/>

</div>
